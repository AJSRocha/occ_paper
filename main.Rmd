...
merge((lota16 %>% mutate(MES = gsub('(?<=\\b|-)0',
      ...
      all.x = T, all.y = F) %>%
             ...
             n_amplia = INDIF * land_kg.x/PESO_AM) %>%
    group_by(REGIAO, PORTO_NOME, GEAR, ANO, MES, C_CLASSE) %>%
    summarise(desemb = unique(land_kg.y),
              desemb_viag = sum(land_kg.x),
              n = sum(n_amplia),
              n_final = n_amplia * desemb / desemb_viag) %>%  #a partir daqui é o check
  filter(REGIAO == "27.a.c.n") %>%
  filter(GEAR == "MIS") %>%
  filter(MES == "5") %>%
  filter(ANO == "2012") %>%
  group_by(C_CLASSE) %>%
  summarise(freq = sum(n_final)/1000) %>% View

  

# para ver as obersvaçoes que nao tem desembarques associados
# %>% filter(is.na(land_kg.y)) %>% View
      ...
      group_by(PORTO_NOME, ARTE_EU,ANO,MES) %>%
      summarise()
#acabar
